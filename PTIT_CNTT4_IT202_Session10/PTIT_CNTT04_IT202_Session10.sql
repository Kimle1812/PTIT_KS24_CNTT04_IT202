...
VALUES ('nguyenviet', 'Nguyễn Việt Hoàng', 'Nam', 'viet@gmail.com', '123', '1996-06-15', 'Hà Nội');
SELECT * FROM view_users_firstname;

DELETE FROM users WHERE username = 'nguyenviet';
SELECT * FROM view_users_firstname;
...
SELECT u.full_name, v.total_user_post FROM users u JOIN view_user_post v ON u.user_id = v.user_id;
...
CREATE INDEX idx_created_at_user_id ON posts (create_at, user_id);
...
SELECT status, COUNT(user_id) AS user_count FROM view_user_activity_status
GROUP BY status
...
SELECT post_id, username, content, total_likes, total_comments,(total_likes + total_comments) AS total_interaction FROM view_popular_posts
...
SELECT u.user_id, u.username, u.full_name, vua.total_posts, vua.total_comments FROM users u
JOIN view_user_activity vua ON u.user_id = vua.user_id
WHERE vua.total_posts > 5 AND vua.total_comments > 20
ORDER BY vua.total_comments DESC
LIMIT 5;
