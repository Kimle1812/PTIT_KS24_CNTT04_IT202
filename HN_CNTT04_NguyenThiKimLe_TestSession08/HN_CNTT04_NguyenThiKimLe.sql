...
SELECT price_per_day FROM rooms 
...
SELECT room_type, COUNT(*) AS so_luong_phong FROM rooms 
GROUP BY room_type;
...
SELECT g.guest_name, r.room_type, b.check_in, b.check_out FROM bookings b
JOIN guests g ON b.guest_id = g.guest_id
JOIN rooms r ON b.room_id = r.room_id;

-- Cho biết mỗi khách đã đặt phòng bao nhiêu lần
SELECT g.guest_name , COUNT(b.booking_id) AS so_lan_dat FROM guests g
...
SELECT r.room_id, r.room_type, SUM((b.check_out - b.check_in) * r.price_per_day) AS doanh_thu FROM bookings b
JOIN rooms r ON b.room_id = r.room_id
GROUP BY r.room_id, r.room_type;
-- Hiển thị tổng doanh thu của từng loại phòng
SELECT r.room_type, SUM((b.check_out - b.check_in) * r.price_per_day) AS tong_doanh_thu FROM bookings b
JOIN rooms r ON b.room_id = r.room_id
GROUP BY r.room_type;
-- Tìm những khách đã đặt phòng từ 2 lần trở lên
SELECT g.guest_name, COUNT(*) AS so_lan_dat FROM bookings b
JOIN guests g ON b.guest_id = g.guest_id
GROUP BY g.guest_id, g.guest_name
HAVING COUNT(*) >= 2;
-- Tìm loại phòng có số lượt đặt phòng nhiều nhất
SELECT r.room_type, COUNT(*) AS so_luot_dat FROM bookings b
JOIN rooms r ON b.room_id = r.room_id
GROUP BY r.room_type
