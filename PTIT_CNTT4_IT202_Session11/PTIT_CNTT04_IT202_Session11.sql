...
CALL CreatePostWithValidation(1, 'ab', @result); 
SELECT @result;

CALL CreatePostWithValidation(1, 'Nội dung của bài viết', @result); 
SELECT @result;
...
BEGIN 
    ...
    SELECT f.friend_id, CONCAT('Người dùng ', p_user_id, ' vừa đăng bài mới: ', p_content), NOW()
    FROM friends f
    WHERE f.user_id = p_user_id;
END $$
DELIMITER ;

CALL NotifyFriendsOnNewPost(1, 'Bài viết mới từ user 1');
